...
This is probably most useful when x and y have different dimensions. For example, we can treat an extra dimension as a batch dimension:

```{python}
z = torch.rand(3, 3, 5)
ans = torch.einsum("ij,jkl", x, z)
